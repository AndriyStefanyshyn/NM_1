...
public class Finder {
        ...
        if(getY(To)*getY(From) < 0) {
            ...
            while(true) {
                ...
                if(Math.abs(To - From) < Eps || getY(x) == 0) {
                    return ("Поділу: х = " + new BigDecimal(x).setScale(3, RoundingMode.HALF_UP).doubleValue() + "; і = " + i);
                }
                else {
    ...
    public static String chord(double From, double To, double Eps) {
        if(getY(To)*getY(From) < 0) {
            ...
            if(Math.abs(To - From) < Eps) {
                x = (To + From)/2f;
                return ("Хорд: х = " + new BigDecimal(x).setScale(3, RoundingMode.HALF_UP).doubleValue() + "; і = " + i);
            }
    ...
    public static String neuton(double From, double To, double Eps) {
        if(getY(To)*getY(From) < 0) {
            ...
            if(Math.abs(To - From) < Eps) {
                x = (To + From)/2f;
                return ("Ньютона: х = " + new BigDecimal(x).setScale(3, RoundingMode.HALF_UP).doubleValue() + "; і = " + i);
            }
            if(!(getYDD(To)*getY(To) > 0)) {
                double temp = From;
                From = To;
    ...
    public static String iter(double From, double To, double Eps) {
        if(getY(To)*getY(From) < 0) {
            ...
            if(Math.abs(To - From) < Eps) {
                x = (To + From)/2f;
                return ("Ітерацій: х = " + new BigDecimal(x).setScale(3, RoundingMode.HALF_UP).doubleValue() + "; і = " + i);
            }
            x0 = (To + From)/2f;
            while(true) {
                x = getFi(x0);
